...
nBlock_Meter::nBlock_Meter(PinName pinPWM, uint32_t Frequency, float meterVcc, float meterVmax, float leftValue, float rightV): 
    _pwm(pinPWM) {
	duty_float = 0.0;
	is_updated = 0;
...
void nBlock_Meter::triggerInput(nBlocks_Message message) {
	...
	if (message.dataType == OUTPUT_TYPE_FLOAT) {
		...
		if(is_updated){							// scale the output
			...
			tmp = tmp / (rightV - leftValue);		// normalize so the desired [voltage_span value] becomes 1 (100% of the meter scale)
			tmp = tmp * meterVmax/meterVcc		// Reduce for Vcc larger that the meter Max voltage (100% doesn't exceed meter scale)
			duty_float = tmp;
		}
	}
